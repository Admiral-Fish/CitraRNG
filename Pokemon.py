...
class Pokemon:
    ...
    def SpeciesNum(self):
        return struct.unpack("<H", self.data[0x8:0xA])[0]

    def Species(self):
        return species[self.SpeciesNum()]

    def HeldItem(self):
        val = struct.unpack("<H", self.data[0xA:0xC])[0]
        return items[val]
    ...
    def Ability(self):
        val = struct.unpack("B", self.data[0x14:0x15])[0]
        return abilities[val]
    ...
    def Move2(self):
        move_num = struct.unpack("<H", self.data[0x5C:0x5E])[0]
        return moves[move_num]
    
    def Move3(self):
        move_num = struct.unpack("<H", self.data[0x5E:0x60])[0]
        return moves[move_num]
    
    def Move4(self):
        move_num = struct.unpack("<H", self.data[0x60:0x62])[0]
        return moves[move_num]

    def Move1PP(self):
        return struct.unpack("B",  self.data[0x62:0x63])[0]

    def Move2PP(self):
        return struct.unpack("B",  self.data[0x63:0x64])[0]

    def Move3PP(self):
        return struct.unpack("B",  self.data[0x64:0x65])[0]

    def Move4PP(self):
        return struct.unpack("B",  self.data[0x65:0x66])[0]

    def IV32(self):
        return struct.unpack("<I", self.data[0x74:0x78])[0]
    ...
    def HTFriendship(self):
        return struct.unpack("B", self.data[0xA2:0xA3])[0]

    def OTFriendship(self):
        return struct.unpack("B", self.data[0xCA:0xCB])[0]
    ...
    def Level(self):
        return struct.unpack("B", self.data[0xEC:0xED])[0]

    def HP(self):
        return struct.unpack("<H", self.data[0xF2:0xF4])[0]
    
    def Atk(self):
        return struct.unpack("<H", self.data[0xF4:0xF6])[0]
    
    def Def(self):
        return struct.unpack("<H", self.data[0xF6:0xF8])[0]
    
    def SpA(self):
        return struct.unpack("<H", self.data[0xF8:0xFA])[0]
    
    def SpD(self):
        return struct.unpack("<H", self.data[0xFA:0xFC])[0]
...
moves = [ "None", "Pound", "Karate Chop", "Double Slap", "Comet Punch", "Mega Punch", "Pay Day", "Fire Punch", "Ice Punch", "Thunder Punch",
...
abilities = [ "None", "Stench", "Drizzle", "Speed Boost", "Battle Armor", "Sturdy", "Damp", "Limber", "Sand Veil", "Static", "Volt Absorb",
